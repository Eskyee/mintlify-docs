...
<Note>All four mission control endpoints require bearer token (API key) authentication and accept a `userId` query parameter. The web proxy uses session authentication and resolves the user ID from the session before forwarding to these backend endpoints. The fleet graph and traces endpoints return safe default responses for unauthenticated requests instead of an error.</Note>
