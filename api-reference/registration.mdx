...
Validates a bearer token and returns the associated user information. No session authentication is required — the API key is passed in the `Authorization` header.

### Headers

| Header | Required | Description |
|--------|----------|-------------|
| `Authorization` | Yes | Bearer token in the format `Bearer YOUR_API_KEY`. The key must be at least 10 characters. |
...
| `userId` | string | User identifier derived from the key |
...
| Code | Description |
|------|-------------|
| 401 | Missing or invalid API key. The key must be provided as a bearer token and be at least 10 characters long. |
