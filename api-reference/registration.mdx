...
Registers a Home mode installation. Requires the `x-user-email`, `x-user-id`, and `x-user-role` headers to identify the caller. These headers are set by the web proxy when a session is active.
...
| 401 | Unauthorized — missing required identity headers |
...
Registers a Link mode installation that connects an existing OpenClaw instance. Requires the `x-user-email`, `x-user-id`, and `x-user-role` headers to identify the caller.
...
Returns all registered installations across all deployment modes. Requires the `x-user-email`, `x-user-id`, and `x-user-role` headers to identify the caller.
