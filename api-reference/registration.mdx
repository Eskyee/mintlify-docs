...
This endpoint requires the `authenticate` middleware. The request must include identity headers (`x-user-email`, `x-user-id`, `x-user-role`) set by the authentication layer. Unauthenticated requests receive a `401` or `403` response.
