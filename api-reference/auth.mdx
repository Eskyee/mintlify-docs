...
| 400 | Missing nonce in SIWE message — the message must include a nonce obtained from [`GET /api/auth/nonce`](#get-nonce) |
| 401 | Invalid nonce — the nonce in the SIWE message does not match the server-issued nonce |
| 401 | Address mismatch — the wallet address in the SIWE message does not match the `address` field |
