...
<Warning>The AI route middleware (`/api/ai/chat`) reads user context from headers separately from this middleware. Access control on AI routes is enforced by the plan middleware, which checks `x-user-plan` and `x-stripe-subscription-id`.</Warning>
...
|-------|------|-------------|
| `options` | object | WebAuthn `PublicKeyCredentialCreationOptions` to pass to the browser credentials API |
| `challenge` | string | Server-generated challenge. Send this back in the verify request. |

```json
{
  "options": {
    "rp": { "id": "agentbot.raveculture.xyz", "name": "Agentbot" },
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `credential` | object | Yes | The credential object returned by `navigator.credentials.create()` |
| `challenge` | string | Yes | The challenge returned by the registration options endpoint |
...
| Field | Type | Description |
|-------|------|-------------|
| `options` | object | WebAuthn `PublicKeyCredentialRequestOptions` to pass to `navigator.credentials.get()` |
| `challenge` | string | Server-generated challenge. Send this back in the verify request. |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `response` | object | Yes | The assertion response from `navigator.credentials.get()`. Must include `id` (credential ID) and `signCount`. |
| `challenge` | string | Yes | The challenge returned by the authentication options endpoint |
...
<Warning>The platform uses `coinbaseWallet({ preference: 'smartWalletOnly' })` as its only wallet connector. Injected wallets (such as MetaMask) are not supported. Users must connect through a Coinbase Smart Wallet on the Base network.</Warning>
