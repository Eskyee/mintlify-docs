...
The [`POST /api/provision`](/api-reference/agents#provision-with-channel-tokens) endpoint supports an admin fallback when the session is missing (for example, after a Stripe checkout redirect loses the session cookie). When `getServerSession` returns null, the endpoint checks the `email` field in the request body against `ADMIN_EMAILS`. If the email matches, the request proceeds without a session. This fallback only applies to admin users on the provisioning endpoint — all other session-authenticated endpoints still require a valid session.
