...
<Warning>All `/api/render-mcp/*` endpoints require bearer token authentication. Include the API key in the `Authorization` header as `Bearer YOUR_API_KEY`.</Warning>

<Note>These endpoints are informational. Most users should use the official Render MCP Server Docker image directly. See the [MCP page](/mcp) for Agentbot's own MCP server.</Note>
