...
When the soul service cannot be reached, the endpoint returns HTTP `200` with a degraded response. The `degraded` field is set to `true` and all colony fields use safe default values so clients can render a fallback UI without special error handling.
...
| `root.dashboardUrl` | string | Soul dashboard URL (replaces the former top-level `dashboard_url`) |
| `root.serviceUrl` | string | Soul service URL (replaces the former top-level `soul_url`) |

<Note>Prior to this change, a soul service failure returned HTTP `503` with top-level `soul_url` and `dashboard_url` fields. The endpoint now returns HTTP `200` with the full colony response shape (zeroed out) so that clients do not need to handle a separate error status code. The `soul_url` and `dashboard_url` fields have been moved into the `root` object as `serviceUrl` and `dashboardUrl` respectively.</Note>
