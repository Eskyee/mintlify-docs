...
<Note>The endpoint tries each configured soul host in order and uses the first healthy one. A host is considered healthy when its `/soul/status` path returns a JSON object containing an `active` field within 4 seconds. If no host passes the health check, the degraded response is returned. Prior to this change, only a single soul host was attempted and a failure returned HTTP `503` with top-level `soul_url` and `dashboard_url` fields. The endpoint now returns HTTP `200` with the full colony response shape (zeroed out) so that clients do not need to handle a separate error status code.</Note>
