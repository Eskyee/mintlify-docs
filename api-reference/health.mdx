...
- Container health is determined by a process-level check (`pgrep`) that verifies the gateway process is running, rather than an HTTP endpoint probe.
- Docker runs the health check every 30 seconds with a 5-second timeout and 3 retries before marking the container as unhealthy.
