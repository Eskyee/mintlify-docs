...
The OpenClaw image (`ghcr.io/openclaw/openclaw:2026.3.13-1`) provides two health endpoints on each agent container:
...
- Docker runs a `HEALTHCHECK` against `http://127.0.0.1:18789/healthz` every 30 seconds with a 5-second timeout and 3 retries before marking the container as unhealthy.
