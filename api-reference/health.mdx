...
| `docker` | string | Docker availability. `available` when the Docker daemon is reachable, `unavailable` otherwise. |
| `provisioning` | string | Agent provisioning capability. `enabled` when Docker is available, `disabled` otherwise. |
| `provider` | string | Provisioning infrastructure provider. Currently always `render`. |

### Response when Docker is unavailable

When the Docker daemon is not reachable, the health endpoint still returns HTTP `200` but reports degraded capabilities:
...
<Note>The backend also probes `/health` on port `18789` for application-level health checks. The `/healthz` and `/readyz` endpoints are provided by the OpenClaw image itself and are available on all agent services.</Note>
...
Receives platform status notifications from Railway's status page. This endpoint processes incident, component, and page status updates and logs them for monitoring. No authentication is required.
