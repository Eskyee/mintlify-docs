...
| `plan` | string | No | Plan tier. Options: `label`, `solo`, `collective`, `network`. Defaults to `free` when omitted, which is not a valid plan and returns `402` with the message `No free tier`. A paid plan must be specified explicitly. |
...
| 400 | Invalid plan value. The `plan` field must be one of `label`, `solo`, `collective`, or `network`. |
