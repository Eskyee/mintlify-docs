...
|------|-------------|
| 401 | Unauthorized |
| 500 | Failed to fetch provisioned agents |
...
<Warning>This endpoint is planned for a future release and is not yet implemented.</Warning>
...
| Code | Description |
|------|-------------|
| 400 | At least one channel token required (Telegram, WhatsApp, or Discord), invalid `aiProvider`, or invalid `plan` |
| 401 | Unauthorized |
...
Returns the message history. No authentication required.
