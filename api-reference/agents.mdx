...
| `tier` | string | No | Subscription tier hint. Options: `label`, `solo`, `collective`, `network`. This value is forwarded to the backend as the `plan` field. When omitted, the backend deployment defaults to `label`. |

<Note>The agent limit is determined by the subscription plan on the authenticated user's account (`label`: 1, `solo`: 3, `collective`: 10, `network`: 100). It cannot be overridden in the request body.</Note>
...
| 429 | Agent limit reached for your plan. Response includes `current` (agent count) and `limit` fields. Limits: `label` 1, `solo` 3, `collective` 10, `network` 100. Users without a recognized plan default to a limit of 1. |
...
<Warning>This endpoint currently returns mock data. Log entries are generated placeholders, not real agent logs. A future release will connect this endpoint to the backend logging service.</Warning>
...
```json
{
  ...
  "healMessage": "",
  "openclawVersion": "2026.03.14.002"
}
```
...
```json
{
  ...
  "previousImage": "ghcr.io/openclaw/openclaw:2026.3.13",
...
Returns the gateway token for the agent. If no token exists, a new cryptographically random token is generated using 32 bytes of entropy (returned as a 64-character hex string).
...
Provisions a new agent with messaging channel tokens. Authentication is not required, but when a session is active the user's email is automatically resolved from it. At least one channel token is required.
...
| `container.port` | number | Host port mapped to the container |
...
Each `aiProvider` value maps to a default model and base URL:

| Provider | Default model | Fallback model | Base URL |
|----------|--------------|----------------|----------|
...
Each provider now includes a fallback model that is used automatically when the primary model is unavailable or returns an error.
