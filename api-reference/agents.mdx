...
| `container.controlUiUrl` | string | Auto-connect URL for the OpenClaw Control UI. Includes `gatewayUrl` and `token` as query parameters (for example, `https://service.up.railway.app/?gatewayUrl=wss%3A%2F%2Fservice.up.railway.app&token=...`). The `gatewayUrl` parameter provides the WebSocket endpoint so the Control UI can store it before its onboarding check runs, preventing a redirect to `/settings` that would discard the token. Only present when a gateway token is configured. |
