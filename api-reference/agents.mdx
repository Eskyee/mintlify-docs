...
When the session is missing or invalid (for example, after a Stripe checkout redirect loses the session cookie), the endpoint falls back to checking the `email` field in the request body against the configured `ADMIN_EMAILS`. If the email matches an admin, the request proceeds without a session. Non-admin users without a valid session receive a `401` error.
...
| `email` | string | No | User email address. When omitted, the email is automatically populated from the authenticated session if one exists. When the session is missing, this field is checked against `ADMIN_EMAILS` to allow admin users to provision without a session (see [admin session fallback](#admin-session-fallback) above). The resolved email is forwarded to the backend in the `X-User-Email` header. |
| `stripeSubscriptionId` | string | No | Stripe subscription ID from checkout |

<Note>When a session is active, the server resolves the user email from the session via `getServerSession`. If the `email` field is also present in the request body, the request body value takes precedence. When no session is present, the `email` field in the request body is checked against `ADMIN_EMAILS` — if it matches, the request is allowed to proceed. The resolved email is sent to the backend provisioning service in the `X-User-Email` header.</Note>
