...
Admin status is determined by checking both the session email and the request body `email` field against `ADMIN_EMAILS`. If either matches, the caller is treated as an admin. This dual check ensures that admin users whose session email differs from their configured admin email are still recognized. See [admin check](#admin-check) in the `POST /api/provision` section for the full resolution order.
...
Admin status is determined using a dual check against the configured `ADMIN_EMAILS`:

1. **Session email** — the email resolved from the authenticated session is checked first.
2. **Body email fallback** — if the session email does not match an admin, the `email` field in the request body is also checked. This covers cases where the session email differs from the body email (for example, when a user authenticates with one email but provisions on behalf of another admin email).

When the session is missing or invalid entirely (for example, after a Stripe checkout redirect loses the session cookie), the endpoint falls back to checking only the `email` field in the request body against `ADMIN_EMAILS`. If it matches, the request proceeds without a session. Non-admin users without a valid session receive a `401` error.
...
| `email` | string | No | User email address. When omitted, the email is automatically populated from the authenticated session if one exists. This field is checked against `ADMIN_EMAILS` both as a fallback when the session email is not an admin and as the primary check when no session is present (see [admin check](#admin-check) above). The resolved email is forwarded to the backend in the `X-User-Email` header. |
| `stripeSubscriptionId` | string | No | Stripe subscription ID from checkout |

<Note>When a session is active, the server resolves the user email from the session via `getServerSession`. Admin status is then checked against the session email first; if the session email is not in `ADMIN_EMAILS`, the `email` field from the request body is checked as a secondary admin lookup. When no session is present at all, the `email` field in the request body is checked against `ADMIN_EMAILS` — if it matches, the request is allowed to proceed without a session. The resolved email is sent to the backend provisioning service in the `X-User-Email` header.</Note>
...
| `container.port` | number | Mapped port on the host for the agent gateway |
| `container.gatewayUrl` | string | Local URL to reach the agent gateway (for example, `http://127.0.0.1:18789`) |
...
| 401 | Authentication required. Returned when no session is present and the `email` in the request body does not match a configured admin email (or is missing). |
| 402 | Active subscription required (web proxy). Returned when the authenticated user does not have an active subscription and is not an admin (neither the session email nor the body email matches `ADMIN_EMAILS`). The response body includes a `code` field set to `SUBSCRIPTION_REQUIRED`. |
