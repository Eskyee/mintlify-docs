...
| Field | Type | Description |
|-------|------|-------------|
| `container.name` | string | Container name (format: `openclaw-{agentId}`) |
| `container.status` | string | Container status (for example, `running`, `stopped`) |
| `container.port` | number | Port the container is listening on |
| `container.gatewayUrl` | string | Local gateway URL for the agent container (for example, `http://127.0.0.1:10001`) |
