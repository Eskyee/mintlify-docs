...
Resumes a suspended agent service on Render. This calls the Render API's resume endpoint for the agent's service.
...
Suspends the agent's Render service. The service retains its data and configuration and can be resumed later with the [start endpoint](#start-agent).
...
| `plan` | string | No | Plan tier. Options: `solo`, `collective`, `label`, `network`. Defaults to `solo` when omitted. The backend also accepts `free` as a value, but it is rejected with a `402` error — a paid plan must be specified when calling the backend directly. |
...
<Note>The `container` object is included when the backend successfully creates a Render service for the agent. It contains a `serviceId` (the Render service identifier) and `url` (the public URL). If service creation fails (for example, when the Render API is unavailable), provisioning still succeeds and the `container` field is omitted. The agent can operate using API-side processing until the service becomes available. You can check Render API availability using the [backend health endpoint](/api-reference/health#backend-health-check).</Note>

| Field | Type | Description |
|-------|------|-------------|
| `container.name` | string | Render service name |
| `container.status` | string | Service status (for example, `running`, `stopped`, `suspended`) |
| `container.serviceId` | string | Render service ID |
| `container.url` | string | Public URL for the agent service (for example, `https://agentbot-agent-a1b2c3d4e5.onrender.com`) |
...
| 400 | Invalid plan value (backend only). The `plan` field must be one of `solo`, `collective`, `label`, or `network`. The web proxy defaults to `solo` when the plan is omitted, so this error is only returned when calling the backend directly with an unrecognized value (including `free`). |
...
| 402 | Active subscription required (web proxy). <br /><br />**Temporarily disabled.** The server-side subscription check on this endpoint is currently bypassed while an environment configuration issue is resolved. The frontend continues to enforce payment before showing the deploy action. This check will be re-enabled in a future update. <br /><br />When active, this error is returned when the authenticated user does not have an active subscription and is not an admin. The response body includes a `code` field set to `SUBSCRIPTION_REQUIRED`. |
| 402 | Active subscription required (backend). <br /><br />**Temporarily disabled.** The backend Stripe subscription check is currently bypassed while an environment configuration issue (`ADMIN_EMAILS` on Render) is resolved. The frontend continues to enforce payment before showing the deploy action. This check will be re-enabled in a future update. <br /><br />When active, this error is returned when a valid plan is specified but no Stripe subscription ID is provided and the caller is not an admin or tester. The response body includes a `code` field set to `PAYMENT_REQUIRED`. |
