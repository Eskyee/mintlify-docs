...
<Note>The gateway client validates the sync response body. A `2xx` status with `"success": false` (or a body the client otherwise rejects) is treated as a failed sync and surfaces as a `500` from this endpoint, rather than being reported as a successful deploy.</Note>
