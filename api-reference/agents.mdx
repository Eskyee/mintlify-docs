...
| 503 | Web proxy | Managed runtime controls are temporarily disabled |
...
|------|--------|-------------|
| 401 | Both | Unauthorized — missing or invalid authentication |
| 403 | Both | Forbidden — authenticated user does not own this agent. The backend returns `{ "error": "Forbidden — you do not own this agent" }`. The web proxy returns `{ "success": false, "status": "error" }`. |
...
|----------|--------------|----------------|----------|
| `openrouter` | `openrouter/xiaomi/mimo-v2-pro` | `openrouter/openai/gpt-4o-mini` | `https://openrouter.ai/api/v1` |
