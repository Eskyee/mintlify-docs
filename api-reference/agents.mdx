...
<Note>When the backend is unavailable, CPU and memory metrics cannot be retrieved because the agent's `/healthz` and `/readyz` endpoints do not report resource usage. The `cpu` and `memory` fields return `"—"` to indicate that the agent is running but detailed metrics are unavailable.</Note>
