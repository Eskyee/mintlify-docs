...
| `email` | string | No | User email address. Used for admin and tester bypass of payment enforcement. When omitted, the email is automatically populated from the authenticated session if one exists. |
...
<Note>When a session is active, the resolved email (from the request body or the session) is forwarded to the backend provisioning service in the `X-User-Email` header. The backend uses this header as a fallback when the `email` field is not present in the request body.</Note>
...
| 503 | Provisioning temporarily disabled (kill switch active). The response includes `success: false` and an error message directing you to contact support. |
