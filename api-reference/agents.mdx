...
<Note>The web proxy enforces agent limits based on the subscription tier (`starter`: 1, `pro`: 3, `enterprise`: 100). The backend plan middleware enforces separate limits (`label`: 1, `solo`: 3, `collective`: 10, `network`: 100). The backend provisioning route (`POST /api/provision`) enforces its own limits (`solo`: 1, `collective`: 3, `label`: 10, `network`: unlimited). The limit cannot be overridden in the request body.</Note>
...
| `plan` | string | No | Plan tier. Options: `solo`, `collective`, `label`, `network`. Defaults to `free` when omitted, which is not a valid plan and returns `402` with the message `No free tier`. A paid plan must be specified explicitly. |
...
| 400 | Invalid plan value. The `plan` field must be one of `solo`, `collective`, `label`, or `network`. |
| 402 | No free tier. A paid subscription is required. Returned when the plan is `free` (the default when omitted) or unrecognized. The response body includes a `code` field set to `PAYMENT_REQUIRED`. |
