...
| 402 | Active subscription required (web proxy). <br /><br />**Temporarily disabled.** The server-side subscription check on this endpoint is currently bypassed while an environment configuration issue is resolved. The frontend continues to enforce payment before showing the deploy action. The backend provisioning service still enforces its own Stripe subscription check independently (see the `PAYMENT_REQUIRED` error below). This check will be re-enabled in a future update. <br /><br />When active, this error is returned when the authenticated user does not have an active subscription and is not an admin. The response body includes a `code` field set to `SUBSCRIPTION_REQUIRED`. |
| 402 | Active subscription required (backend). Returned when a valid plan is specified but no Stripe subscription ID is provided and the caller is not an admin or tester. The response body includes a `code` field set to `PAYMENT_REQUIRED`. |
