...
2. **Railway direct provisioning** — when all backend URLs fail or are not configured, the endpoint falls back to provisioning the agent container directly via the Railway GraphQL API. This path requires `RAILWAY_API_KEY`, `RAILWAY_PROJECT_ID`, and `RAILWAY_ENVIRONMENT_ID` to be set. The Railway path creates a new Railway service, injects environment variables, sets a TCP proxy start command via the `ServiceInstanceUpdate` mutation, generates a public domain via the `serviceDomainCreate` mutation, and then triggers a deployment. The start command spawns the OpenClaw gateway and creates a TCP proxy that forwards the Railway-assigned port to `127.0.0.1:18789`. The domain is generated before the deploy so that the service has a routable public URL immediately. The returned URL is the direct Railway domain for the service.
...
<Note>The Railway path does not return `subdomain`, `streamKey`, or `liveStreamId`. These fields are only available when the backend Express service handles the request. The `status` field is `deploying` (not `active`) because the Railway service is created asynchronously. The Railway path sets a TCP proxy start command on the service so that the Railway-assigned port is forwarded to the OpenClaw gateway on `127.0.0.1:18789`. The `PORT`, `OPENCLAW_GATEWAY_BIND`, and `OPENCLAW_BIND` environment variables are not injected — `PORT` is managed by Railway, and the bind flags have no effect on OpenClaw.</Note>
...
```json
{
  ...
  "container": {
    ...
    "controlUiUrl": "https://agentbot-agent-a1b2c3d4e5.up.railway.app/#token=your-gateway-token"
  }
}
```
...
| `container.controlUiUrl` | string | Auto-connect URL for the OpenClaw Control UI. The gateway token is passed as a URL hash fragment (for example, `https://service.up.railway.app/#token=...`). Hash fragments are never sent to the server, which prevents server-side redirects from stripping the token. The dashboard client also appends `gatewayUrl` to the hash (for example, `/#token=...&gatewayUrl=wss%3A%2F%2Fservice.up.railway.app`) so the Control UI can store the WebSocket endpoint before its onboarding check runs. Only present when a gateway token is configured. |
