...
```json
{
  ...
  "container": {
    ...
    "controlUiUrl": "https://agentbot-agent-a1b2c3d4e5.up.railway.app/?token=your-gateway-token#token=your-gateway-token"
  }
}
```
...
| `container.controlUiUrl` | string | Auto-connect URL for the OpenClaw Control UI. Includes the gateway token in both the query parameter and the URL fragment (for example, `https://service.up.railway.app/?token=...#token=...`). The query parameter ensures the token survives SPA redirects (such as the Control UI redirecting to `/chat?session=main`), while the fragment provides a fallback. Only present when a gateway token is configured. |
