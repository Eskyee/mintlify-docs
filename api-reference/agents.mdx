...
| 429 | Agent limit reached for your plan. Response includes `current` (agent count) and `limit` fields. Limits: `free` 1, `starter` 1, `pro` 3, `enterprise` 100. Users without a recognized plan are treated as `free` tier. |
...
<Warning>This endpoint is not yet available. It is planned for a future release.</Warning>
