...
```json
{
  ...
  "container": {
    ...
    "status": "running",
    "port": null,
    "gatewayUrl": "http://127.0.0.1:undefined"
  }
}
```
...
| `container.port` | number \| null | Port the container is listening on. Currently `null` when provisioned via the Render API path because the Render service does not expose a local port. |
| `container.gatewayUrl` | string | Local gateway URL for the agent container. Currently set to `http://127.0.0.1:undefined` when provisioned via the Render API path because `port` is not populated. This field is only meaningful for local Docker deployments. |
