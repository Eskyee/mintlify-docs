...
| `container.controlUiUrl` | string | Auto-connect URL for the OpenClaw Control UI. Includes the gateway token in the URL fragment (for example, `https://service.up.railway.app/#token=...`). The token is in the fragment so it is never sent to the server — the Control UI reads it client-side on load. Only present when a gateway token is configured. |
