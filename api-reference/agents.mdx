...
The backend returns a flat array of agent objects (not wrapped in an `agents` key):
...
The backend returns a flat object (not wrapped in an `agent` key):
...
<Note>The web proxy enforces agent limits based on the subscription tier (`starter`: 1, `pro`: 3, `enterprise`: 100). The backend enforces separate plan-based limits (`label`: 1, `solo`: 3, `collective`: 10, `network`: 100). The limit cannot be overridden in the request body.</Note>
...
| `specialization` | string | No | Agent specialization (default: `general`) |
...
Clones an existing agent into a new agent instance. Requires an [x402 payment proof](/payments/x402) to complete. The cloned agent receives a new wallet and is assigned a generation number one higher than its parent.
...
| `generation` | number | No | Current generation of the parent agent (default: `1`). The cloned agent is assigned `generation + 1`. |
| `paymentProof` | object | Yes | On-chain payment proof for the clone fee |
| `paymentProof.transactionHash` | string | Yes | Transaction hash of the payment |
| `paymentProof.amount` | string | Yes | Amount paid (for example, `"1.0"`) |
| `paymentProof.currency` | string | Yes | Currency used (for example, `"pathUSD"`) |
| `paymentProof.chainId` | number | Yes | Chain ID of the transaction (for example, `4217` for Tempo) |
| `paymentProof.from` | string | Yes | Sender wallet address |
| `paymentProof.to` | string | Yes | Recipient wallet address |
| `paymentProof.timestamp` | number | Yes | Unix timestamp of the payment |
...
{
  "success": true,
  "agentId": "agent_1710806400000_a1b2c3",
...
|-------|------|-------------|
| `agentId` | string | Unique identifier for the new agent |
| `walletAddress` | string | Wallet address assigned to the cloned agent |
| `parentAgentId` | string | ID of the parent agent |
| `generation` | number | Generation number of the cloned agent (parent generation + 1) |
| `transactionHash` | string | Transaction hash from the payment proof |
| `specialization` | string | Specialization assigned to the clone |
| `status` | string | Current status. Always `provisioning` on success. |
| `message` | string | Human-readable confirmation message |
...
## Clone endpoint status

```http
GET /api/agents/clone
```

Returns the current status of the clone service, including pricing and protocol information. No authentication required.
...
Streams live logs from an agent's container via Server-Sent Events (SSE). Requires API key authentication (backend). The stream follows the Docker container logs in real time without requiring SSH access.

When a client connects, existing buffered log lines are sent immediately, followed by new lines as they arrive. The server maintains a buffer of up to 500 lines per agent.
...
The response uses the `text/event-stream` content type. Each event is a JSON object sent as an SSE `data` field.
...
```
data: {"type": "connected", "agentId": "agent_123"}
```

#### Log line event

Sent for each log line (stdout). Lines from stderr are prefixed with `[stderr]`:

```
data: {"type": "line", "text": "2026-03-22T00:00:00Z [gateway] Agent started"}
```

#### Exit event

Sent when the container process exits:
...
| Field | Type | Description |
|-------|------|-------------|
| `type` | string | Event type: `connected`, `line`, or `exit` |
| `agentId` | string | Agent identifier (present on `connected` events) |
| `text` | string | Log line content (present on `line` events) |
| `code` | number | Process exit code (present on `exit` events) |

### Stream behavior

- The server buffers the most recent 500 log lines per agent.
- Multiple clients can connect to the same agent's stream simultaneously.
- When all clients disconnect, the stream continues for a 30-second grace period before stopping. If a client reconnects within this window, the stream resumes without interruption.
- Docker `--tail 100` is used when initially attaching to the container logs.
...
```bash
curl -N https://backend.example.com/api/logs/agent_123/stream \
...
Returns recent buffered log lines for an agent without streaming. Requires API key authentication (backend). Returns an empty array if no active stream exists for the agent.
...
```json
{
  "agentId": "agent_123",
  "lines": [
    "2026-03-22T00:00:00Z [gateway] Agent started",
    "2026-03-22T00:00:01Z [gateway] Health check passed"
  ],
  "streaming": true,
  "clients": 2
}
...
|-------|------|-------------|
| `agentId` | string | Agent identifier |
| `lines` | string[] | Buffered log lines (up to 500) |
| `streaming` | boolean | Whether an active log stream exists for this agent |
| `clients` | number | Number of SSE clients currently connected to this agent's stream |
...
Stops the live log stream for an agent and disconnects all SSE clients. Requires API key authentication (backend). Returns success even if no active stream exists.
...
Returns all currently active log streams across all agents. Requires API key authentication (backend).
...
| Field | Type | Description |
|-------|------|-------------|
| `streams` | array | List of active log streams |
| `streams[].agentId` | string | Agent identifier |
| `streams[].clients` | number | Number of connected SSE clients |
| `streams[].lines` | number | Number of buffered log lines |
...
| `backupPath` | string | Path to the pre-update backup |
| `openclawVersion` | string | Current OpenClaw runtime version |
...
Returns the gateway token for the agent. If no token exists, a new cryptographically random token is generated using 32 bytes of entropy (returned as a 64-character hex string). Tokens generated during initial provisioning use 24 bytes (48 hex characters). Tokens generated by the container entrypoint (when the `OPENCLAW_GATEWAY_TOKEN` environment variable is not set) use 16 bytes (32 hex characters).

```json
{
  "token": "a1b2c3d4e5f6...48 or 64 hex characters"
}
```
...
<Warning>This endpoint is rate limited to 5 requests per minute per IP. Exceeding this limit returns `429` with the message `Deployment rate limit exceeded.`</Warning>
...
| `plan` | string | No | Plan tier. Options: `label`, `solo`, `collective`, `network`. Defaults to `free` when omitted, which is not a valid plan and returns `400 Invalid plan`. A paid plan must be specified explicitly. |
...
<Note>The `container` object is included when the backend successfully creates a Docker container for the agent. If container creation fails, provisioning still succeeds and the `container` field is omitted. The agent can operate using API-side processing until a container becomes available.</Note>
...
| Code | Description |
|------|-------------|
| 400 | At least one channel token required (Telegram, WhatsApp, or Discord), or invalid `aiProvider` or `plan` value |
| 400 | Invalid plan value. The `plan` field must be one of `label`, `solo`, `collective`, or `network`. The default value `free` is not a recognized plan. |
| 402 | No free tier. A paid subscription is required. The response body includes a `code` field set to `PAYMENT_REQUIRED`. |
| 402 | Agent limit reached for your plan. The response body includes a `code` field set to `AGENT_LIMIT_REACHED`, along with `current` (current agent count) and `limit` (maximum allowed) fields. |
| 500 | Internal server error |
...
Returns the current OpenClaw runtime version. No authentication required.
