...
The request is proxied to the backend provisioning service. The backend may create a dedicated Mux live stream and enqueue the provisioning job for asynchronous processing.
...
| `plan` | string | No | Plan tier (default: `solo`). Options: `solo`, `collective`, `label`, `network`. The `free` value is deprecated and now returns `402 Payment Required`. |
...
<Warning>The following request fields are deprecated and no longer accepted: `whatsappPhoneNumberId`, `whatsappBusinessAccountId`, `discordGuildId`, `discordChannelId`, `stripeSubscriptionId`.</Warning>
...
<Note>This is a backend-only endpoint. It deploys an agent container directly and requires a `Content-Type: application/json` header. Requires bearer token authentication.</Note>
...
| `config.plan` | string | No | Plan tier (default: `solo`). The `free` value is deprecated and now returns `402 Payment Required`. |
