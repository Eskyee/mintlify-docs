...
Sends a message to the caller's deployed agent. The server connects to the agent's OpenClaw Gateway via WebSocket, performs the challenge-response authentication handshake, delivers the message, and returns the agent's reply.
...
| `topic` | string | No | Conversation topic. Defaults to `main`. |
...
<Note>The server opens a WebSocket connection to the agent's gateway for each request. Responses time out after 30 seconds. If the gateway is unreachable or the agent does not respond within this window, a `500` error is returned.</Note>
