...
Returns the gateway token for the agent. If no token exists, a new cryptographically random token is generated using 32 bytes of entropy (returned as a 43-character base64url string). Tokens generated during initial provisioning use 32 bytes of entropy encoded as base64url. Tokens generated by the container entrypoint (when the `OPENCLAW_GATEWAY_TOKEN` environment variable is not set) use 16 bytes (32 hex characters).

```json
{
  "token": "a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d"
}
```
