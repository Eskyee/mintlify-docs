...
Updates the OpenClaw service start command on Railway and triggers a redeploy. The updated start command writes a gateway configuration file with `gateway.trustedProxies` and `gateway.controlUi.allowedOrigins` before launching the OpenClaw process. This resolves "origin not allowed" WebSocket rejections when accessing the Control UI.

Authenticates using the `INTERNAL_API_KEY` passed as a query parameter. This endpoint is intended for one-time administrative use to apply configuration fixes to a running service.
...
{
  "ok": true,
  "message": "Start command updated and redeploy triggered"
}
```
...
| Code | Description |
|------|-------------|
| 401 | Unauthorized — the `key` query parameter is missing or does not match `INTERNAL_API_KEY` |
| 500 | Update or redeploy failed |
