...
<Note>The GET endpoint uses `/verification` while POST and DELETE use `/verify`.</Note>
...
| `verified` | boolean | Yes | Whether the agent is verified |
...
| `plan` | string | No | Plan tier (default: `free`, which returns `402 Payment Required`). Options: `label`, `solo`, `collective`, `network`. A paid plan must be specified explicitly. |
...
| 402 | No free tier. A paid plan (`label`, `solo`, `collective`, or `network`) must be specified. Returned when the `plan` field is omitted or set to `free`. The response body includes a `code` field set to `PAYMENT_REQUIRED`. |
| 500 | Internal server error |
