...
| Field | Type | Description |
|-------|------|-------------|
| `image` | string | New container image. The platform default is pinned to `ghcr.io/openclaw/openclaw:2026.3.13-1`. |
...
Returns the gateway token for the agent. If no token exists, a new cryptographically random token is generated using 32 bytes of entropy (returned as a 64-character hex string). Note that tokens generated during initial provisioning use 24 bytes (48 hex characters) instead.
