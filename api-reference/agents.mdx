...
| Field | Type | Description |
|-------|------|-------------|
| `image` | string | New service image. The platform default is `ghcr.io/openclaw/openclaw:2026.4.7`. |
...
All web proxy lifecycle endpoints return `503` when managed runtime controls are disabled:
...
| 503 | Web proxy | Managed runtime controls are temporarily disabled. This applies to all lifecycle actions: start, stop, restart, update, repair, and reset-memory. |
