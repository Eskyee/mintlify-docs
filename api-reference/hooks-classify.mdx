...
5. For `dangerous` tier results, the dashboard polls [`GET /api/permissions`](/api-reference/permissions#list-pending-permission-requests) for pending requests
6. The user approves or rejects via [`POST /api/permissions`](/api-reference/permissions#submit-permission-decision)
