...
| `config.agent.model` | string | Default AI model identifier |
| `config.agent.workspace` | string | Path to the agent's workspace directory |
| `config.agent.thinkingDefault` | string | Default thinking mode (`"auto"`, `"on"`, `"off"`) |
| `config.agent.timeoutSeconds` | number | Maximum time in seconds before the agent times out |
| `config.agent.heartbeat.every` | string | Heartbeat interval (e.g. `"30m"`, `"0m"` to disable) |
...
| `config.session.reset.mode` | string | Reset mode (e.g. `"daily"`) |
| `config.session.reset.atHour` | number | Hour of day (0–23) for daily resets |
| `config.session.reset.idleMinutes` | number | Minutes of inactivity before automatic reset |
