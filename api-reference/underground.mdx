...
|-----------|------|----------|-------------|
| `userId` | number | Yes | User identifier for authorization. The value is cast to a number before being passed to the wallet service. |
...
Creates and executes a royalty split. The split is recorded in the database and processed inline. Requires bearer token authentication.
...
| `status` | string | `executed` — the split is processed synchronously when the request is made |
