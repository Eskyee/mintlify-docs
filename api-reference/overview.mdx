...
| `/api/openclaw/instances/:id/stats` | GET | Get instance service stats (backend only, requires auth) |
| `/api/deployments` | POST | Deploy an agent service (backend only, requires auth) |
