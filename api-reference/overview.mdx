...
| Deployments | `POST /api/deployments`, `POST /api/provision` | 5 req/min per IP | `Deployment rate limit exceeded.` |
...
| `/api/chat` | 60/min |
| `/api/instance/*` | 30/min |
| `/api/provision` | 5/min (deployment limiter) |
...
| `/api/register-home` | POST | Register a Home mode installation (backend only, requires auth) |
| `/api/register-link` | POST | Register a Link mode installation (backend only, requires auth) |
| `/api/installations` | GET | List registered installations (backend only, requires auth) |
...
| `/api/invite/generate` | POST | Generate an invite code (backend only, no auth) |
