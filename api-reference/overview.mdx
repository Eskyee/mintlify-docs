...
| 402 | Payment required — a paid subscription is mandatory for all agent provisioning and usage. The minimum plan is `solo`. Also returned when an [MPP](/payments/mpp) credential is missing for gateway requests. Requests using the deprecated `free` plan value receive this status. **Note:** The subscription checks on the provision endpoints (both web proxy and backend) are temporarily bypassed while an environment configuration issue is resolved. The frontend continues to enforce payment. |
...
| `/api/user/openclaw` | GET | Get the authenticated user's OpenClaw URL and instance ID |
...
| `/api/webhooks/railway-status` | POST | Railway platform status webhook receiver |
| `/api/webhooks/railway-status` | GET | Railway status webhook health check |
