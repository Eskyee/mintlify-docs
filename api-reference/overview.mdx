...
| `/api/openclaw/ensure-compatibility` | POST | Check and fix OpenClaw 2026.4.2 compatibility (session auth) |
| `/api/support/heal-token` | POST | Auto-heal (regenerate) gateway tokens (session auth) |
| `/api/support/diagnostics` | GET | Platform diagnostics snapshot (session auth) |
