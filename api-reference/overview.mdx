...
| `/api/openclaw/ensure-compatibility` | POST | Ensure agent setup is compatible with OpenClaw 2026.4.7 (auto-migrates) |
| `/api/support/heal-token` | POST | Auto-heal (regenerate) gateway token for the authenticated user |
| `/api/openclaw-version` | GET | Get OpenClaw runtime version (web proxy, fallback `2026.4.7`) |
...
| `/api/basefm/streams` | POST | Create a Mux live video + audio stream (RAVE token-gated, 2h sessions) |
| `/api/basefm/streams` | GET | Check active DJ session status and remaining time |
| `/api/basefm/streams` | DELETE | End an active DJ session |
