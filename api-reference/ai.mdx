...
<Note>These endpoints are internal backend endpoints and are not exposed through the web application's `/api` routes. All endpoints in this group require bearer token (API key) authentication, including the health check and model listing endpoints. The chat endpoint additionally requires a valid subscription plan. All POST requests must include the `Content-Type: application/json` header.</Note>
