...
<Warning>The `label` plan has the smallest model set (3 models) and the lowest agent limit in the middleware (1 agent), but the provisioning endpoint allows up to 10 agents on `label`. This is because `label` is designed for label operations where more agents are required but fewer model choices are needed.</Warning>
