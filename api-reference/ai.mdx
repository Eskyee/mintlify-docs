...
<Note>These endpoints are internal backend endpoints and are not exposed through the web application's `/api` routes. Most `/api/ai/*` endpoints require bearer token (API key) authentication. However, the health check, model listing, and per-provider model listing endpoints are **intentionally public** and do not require authentication. The chat and cost estimation endpoints additionally require a valid subscription plan. All POST requests must include the `Content-Type: application/json` header.</Note>
...
**Public endpoint** — no authentication required. Returns the availability status of configured AI providers. This endpoint is intentionally unauthenticated to support status page integrations.
...
**Public endpoint** — no authentication required. Returns all available AI models across providers. This endpoint is intentionally unauthenticated to support pricing and discovery pages.
...
**Public endpoint** — no authentication required. This endpoint is intentionally unauthenticated to support pricing and discovery pages.
...
<Note>The plan middleware (`x-user-plan` header) enforces model access, skill limits, and A2A message quotas. The provisioning endpoint enforces separate agent creation limits: `solo` 1, `collective` 3, `label` 10, `network` unlimited. The provisioning limits determine how many agents you can create, while the middleware limits in the table above apply to per-request AI model access and skill usage.</Note>
