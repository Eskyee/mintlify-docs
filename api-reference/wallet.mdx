...
Returns recent USDC transfer history for a wallet address on Base. The endpoint scans recent blocks for ERC-20 `Transfer` events on the USDC contract, returning both incoming and outgoing transfers.
...
| `indexedAsset` | string | The asset being tracked (`USDC`) |
| `windowBlocks` | string | Number of recent blocks scanned for transfers (`50000`) |
| `transactions` | array | List of USDC transfer events, sorted by block number (newest first) |
| `transactions[].hash` | string | Transaction hash |
| `transactions[].direction` | string | `"sent"` or `"received"` relative to the queried address |
| `transactions[].symbol` | string | Always `"USDC"` |
| `transactions[].amount` | string | Formatted USDC amount (6 decimal places) |
| `transactions[].amountRaw` | string | Raw amount in smallest unit |
...
<Note>The endpoint scans the most recent 50,000 blocks for USDC `Transfer` events involving the queried address. Older transactions outside this window are not returned. For complete history, use the `explorerUrl` from the [wallet balance endpoint](#base-network-balance-query) to link users to Basescan.</Note>

<Warning>The transaction history endpoint has migrated from Tempo to Base. The previous response fields (`note`, `explorerUrl` at the top level) have been replaced. The response now returns real USDC transfer data with `indexedAsset`, `windowBlocks`, and per-transaction `explorerUrl` fields. Only USDC transfers are tracked — native ETH transfers are not included.</Warning>
