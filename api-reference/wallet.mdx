...
Creates a Stripe checkout session for the specified top-up amount. Requires session authentication.
...
| 401 | Unauthorized — no valid session. The response includes a `loginUrl` field with the URL to redirect the user to sign in. |
| 500 | Stripe not configured or checkout creation failed |

##### 401 response

When the user is not authenticated, the response includes a redirect URL so the client can send the user to the sign-in page:

```json
{
  "error": "Please sign in to top up your wallet",
  "loginUrl": "https://agentbot.raveculture.xyz/signup"
}
```
...
| `loginUrl` | string | URL to redirect the user for authentication |

#### Example

```bash
curl -X GET "https://agentbot.raveculture.xyz/api/wallet/top-up?amount=2500" \
