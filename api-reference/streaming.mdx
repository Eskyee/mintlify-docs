...
- The wallet holds at least **1,250,000 BASEFM** tokens on the Base network, or
...
| `stream.accessGrantedBy` | string | How access was granted. Either `basefm` (token balance) or `community-pass` (Builder/Whale community program perk). |
...
| 403 | Insufficient BASEFM tokens or community guest pass. The wallet must hold at least 1,250,000 BASEFM, or the authenticated user must have a Builder or Whale tier community claim. |
