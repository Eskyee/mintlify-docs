...
| `muxStopped` | boolean | Whether the underlying Mux live stream was successfully disabled. Only present when `ended` is `1`. |
| `message` | string | Human-readable status message. `"Session ended and Mux stream disabled"` on full success, or `"Session ended, but Mux stream disable needs attention"` when the session was ended but the Mux API call failed. |
