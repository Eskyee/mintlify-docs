...
Ends all current DJ sessions for the authenticated caller's wallet. Both `active` and `live` sessions are ended and the corresponding Mux live streams are retired. By default, replay assets are deleted to avoid ongoing storage costs. Pass `archive: true` in the request body to preserve replay assets for paid archive storage.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `archive` | boolean | No | When `true`, preserves Mux replay assets for paid archive storage. Requires the owning logged-in Agentbot account, configured archive pricing, and sufficient credits. Defaults to `false`. |

<Note>The request body is optional. When omitted or when `archive` is `false`, the session ends normally and all replay assets are deleted.</Note>

### Response (default — no archive)
...
```json
{
  ...
  "muxStopped": true,
  "archived": false,
  "archiveCreditCost": null,
  "deletedAssetIds": ["asset_abc123"],
  "retainedAssetIds": [],
  "message": "Session ended. Live stream and recent Mux assets deleted to avoid archive cost."
}
```
...
```json
{
  ...
  "muxStopped": true,
  "archived": true,
  "archiveCreditCost": 10,
  "deletedAssetIds": [],
  "retainedAssetIds": ["asset_abc123"],
  "message": "Session archived. Live stream retired and Mux assets kept for paid archive storage."
}
```
...
| `muxStopped` | boolean | Whether all underlying Mux live streams were successfully retired. |
| `archived` | boolean | Whether replay assets were preserved for archive storage. |
| `archiveCreditCost` | number \| null | The number of credits charged for archive retention. `null` when `archived` is `false`. |
| `deletedAssetIds` | array | IDs of Mux assets that were deleted during stream retirement. Empty when archiving. |
| `retainedAssetIds` | array | IDs of Mux assets that were preserved. Empty when not archiving. |
| `message` | string | Human-readable status message describing the outcome. |

<Note>When archive is requested but Mux cleanup fails, any credits that were debited are automatically refunded.</Note>
...
|------|-------------|
| 401 | Unauthorized — no valid session token or user session |
| 402 | Insufficient credits to save a DJ archive. The response includes the required `archiveCreditCost`. |
| 403 | Forbidden — the session token does not match the session owner, or archive was requested by a user who does not own the session |
| 503 | Archive is temporarily unavailable because archive pricing has not been configured on the platform |

<Warning>The `wallet` query parameter is no longer used. Session lookup is now based on the authenticated caller (session token or user session). Requests using the old `?wallet=` parameter are ignored — you must provide authentication instead.</Warning>

<Note>When a wallet has accumulated multiple current sessions (for example, due to a previous lifecycle inconsistency), this endpoint cleans up all of them in a single call and retires each corresponding Mux live stream.</Note>

<Warning>Replay retention is opt-in and requires payment. By default, ending a session deletes all Mux replay assets to prevent unintended storage costs. To preserve replays, you must explicitly pass `archive: true` and have sufficient credits.</Warning>
