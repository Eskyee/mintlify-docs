...
```json
{
  ...
  "ffmpeg": {
    "command": "ffmpeg -re -i INPUT_MEDIA -c:v libx264 ... -f flv rtmp://global-live.mux.com:5222/app/sk-ab12-cd34-ef56",
...
| `ffmpeg.command` | string | A pre-built ffmpeg command template for autonomous agent broadcasting. Replace `INPUT_MEDIA` with the audio/video source path. |
