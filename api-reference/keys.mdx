...
Verifies an API key against its bcrypt hash in the database and returns the associated user information. This endpoint is called by the backend API's [`POST /api/validate-key`](/api-reference/registration#validate-api-key) endpoint to validate keys provided during install and link scripts. No session authentication is required.
...
<Note>On a successful match, the `lastUsed` timestamp on the API key record is automatically updated.</Note>
