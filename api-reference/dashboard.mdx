...
Requires session authentication. Returns all core dashboard data in a single request. This endpoint runs on the Edge Runtime for low-latency responses and uses CDN-level caching with a 5-second stale-while-revalidate window.

### Response headers

| Header | Value | Description |
|--------|-------|-------------|
| `Cache-Control` | `public, s-maxage=5, stale-while-revalidate=30` | CDN cache for 5 seconds, serve stale for up to 30 seconds while revalidating |
...
Requires session authentication. Returns lightweight initialization data for the dashboard, including referral credits, plan information, and OpenClaw connection details.
...
| `gatewayToken` | string \| null | Shared gateway authentication token |
