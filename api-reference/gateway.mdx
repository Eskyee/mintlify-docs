...
| `gateway.auth.token` | Unique 48-character hex string generated from 24 random bytes |
| `gateway.port` | `18789` |
...
|-----------|-------|-------------|
| `gateway.bind` | `lan` | Bind address for the gateway. Set to `lan` for Docker networking (loopback does not receive external traffic). |
...
| `gateway.controlUi.enabled` | `true` | Enable the gateway control UI |
