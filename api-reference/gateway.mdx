...
| `PORT` | `18789` | Port the agent process listens on inside the container. Must match the OpenClaw Gateway port. Set by the provisioning service. |
...
| `OPENCLAW_GATEWAY_BIND` | `0.0.0.0` | Bind address for the gateway process. Set to `0.0.0.0` so Railway (and other reverse proxies) can route traffic into the container. |
| `OPENCLAW_BIND` | `0.0.0.0` | Fallback bind address used when `OPENCLAW_GATEWAY_BIND` is not set. |
