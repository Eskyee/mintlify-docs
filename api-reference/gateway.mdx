...
|----------|---------|-------------|
| `OPENCLAW_GATEWAY_TOKEN` | _(empty)_ | Bearer token for authenticating proxied requests to the internal gateway. When not set, the entrypoint generates a random 64-character hex token. |
...
The production gateway entrypoint configures the following model default:
...
The container image is configured with a Docker `HEALTHCHECK` that probes `GET /api/status` every 15 seconds (5-second timeout, 30-second start period, 3 retries). Railway uses the same path for health monitoring and will restart the container on failure.
