...
This endpoint retrieves the gateway token dynamically from the backend API rather than relying on a single environment variable. The gateway URL is derived from the agent ID (`wss://agentbot-agent-{agentId}-production.up.railway.app`). Per-user `openclawUrl` values from the database are not used for WebSocket routing.
...
| `CONTROL_UI_ORIGIN` | `https://agentbot.raveculture.xyz` | Allowed origin for the gateway control UI. Used to populate `gateway.controlUi.allowedOrigins`. |
...
| `gateway.controlUi.allowedOrigins` | `["https://agentbot.raveculture.xyz"]` | Permitted WebSocket origins. Populated from the `CONTROL_UI_ORIGIN` environment variable. |
