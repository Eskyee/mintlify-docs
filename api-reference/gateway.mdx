...
| `CONTROL_UI_ORIGIN` | `https://agentbot.sh` | Allowed origin for the gateway control UI. Used to populate `gateway.controlUi.allowedOrigins`. |
| `SKIP_SERVICE_READINESS` | `false` | When set to `true`, the gateway starts immediately without waiting for the backend API health check. |
| `SERVICE_HEALTH_URL` | `{AGENTBOT_API_URL}/health` | URL the gateway polls at startup to confirm the backend API is available. Derived from `AGENTBOT_API_URL` by default. |
...
| `gateway.controlUi.allowedOrigins` | `["https://agentbot.sh"]` | Permitted WebSocket origins. Populated from the `CONTROL_UI_ORIGIN` environment variable. |
| `gateway.controlUi.dangerouslyDisableDeviceAuth` | `false` | Device auth is enabled on the production gateway. |
| `gateway.controlUi.dangerouslyAllowHostHeaderOriginFallback` | `false` | Host header fallback is disabled on the production gateway. |
