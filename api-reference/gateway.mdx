---
title: Gateway
description: "Route requests to plugins through the v1 gateway, and chat with agents via the gateway WebSocket proxy."
---
...
Sends a chat message to the caller's deployed agent through the OpenClaw Gateway. The server connects to the agent's gateway via WebSocket, authenticates using the challenge-response handshake, delivers the message, and returns the agent's reply.

This endpoint is similar to [`POST /api/chat`](/api-reference/agents#send-message) but retrieves the gateway token dynamically from the backend API rather than relying on a single environment variable.
...
<Note>The server opens a WebSocket connection per request and times out after 30 seconds. If the gateway rejects the authentication handshake, a `500` error is returned with details from the gateway.</Note>
