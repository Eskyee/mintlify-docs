...
|-----------|-------|-------------|
| `gateway.bind` | `loopback` | Bind address for the gateway. The gateway binds to the loopback interface (`127.0.0.1`). On Railway-provisioned containers, a TCP proxy runs alongside the gateway and forwards traffic from the Railway-assigned `PORT` to `127.0.0.1:18789`, so the gateway does not need to bind to all interfaces. For local Docker containers, the entrypoint uses the `--bind loopback` CLI flag by default. |
...
| `gateway.trustedProxies` | `["127.0.0.1"]` | IP addresses trusted as reverse proxies. Requests from these addresses have their `X-Forwarded-For` headers honored for origin detection. Required for Railway-provisioned containers where a TCP proxy forwards traffic to the gateway on the loopback interface. |
