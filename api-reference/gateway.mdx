...
| `gateway.auth.token` | Unique 32-character hex string auto-generated per container |
| `gateway.port` | `18789` |
...
|-----------|-------|-------------|
| `gateway.bind` | `loopback` | Bind address for the gateway. Set to `loopback` for container isolation — the container port is mapped to the host via Docker. |
...
| `gateway.controlUi.enabled` | `false` | Gateway control UI is disabled in containers. Use the Agentbot dashboard instead. |
...
| `tools.exec.allowedCommands` | Array of whitelisted commands | Commands the agent is permitted to execute (includes `git`, `node`, `npm`, `python`, `curl`, and others) |
| `tools.exec.allowedPaths` | `["~/.openclaw/workspace", "/tmp", "/home/node"]` | Filesystem paths the agent can access |
