...
| `gateway.auth.token` | Unique 48-character hex string (24 bytes) auto-generated per container |
| `gateway.port` | `18789` |
...
|-----------|-------|-------------|
| `gateway.bind` | `lan` | Bind address for the gateway. Set to `lan` to listen on all interfaces, which is required for Docker networking — the container port is mapped to the host. |
...
| `tools.exec.allowedCommands` | Array of whitelisted commands | Commands the agent is permitted to execute (includes `git`, `node`, `npm`, `python3`, `curl`, `docker`, and others) |
