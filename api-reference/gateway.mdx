...
| `gateway.auth.token` | Unique 48-character hex string (24 bytes of entropy) auto-generated per container |
| `gateway.port` | `18789` |
...
|-----------|-------|-------------|
| `gateway.bind` | `lan` | Bind address for the gateway. Set to `lan` so the container listens on all interfaces, which is required for Docker port mapping. |
...
| `tools.exec.allowedPaths` | `["~/.openclaw/workspace", "/tmp", "/home/node"]` | Filesystem paths the agent can access. The official OpenClaw image runs as the `node` user (`/home/node`). |
