...
When the backend is unreachable or returns an error, the endpoint returns a fallback version of `2026.4.2`.
...
| Field | Type | Description |
|-------|------|-------------|
| `openclawVersion` | string | OpenClaw runtime version from the backend. Falls back to `2026.4.2` when the backend is unavailable or returns an error. |
