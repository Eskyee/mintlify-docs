...
Returns the current OpenClaw runtime version and container image. No authentication required.
...
| `deployedAt` | string | ISO 8601 timestamp of when the version info was served |
...
Returns all running OpenClaw agent containers with their image, status, and metadata. No authentication required.
...
Returns live resource usage for a specific OpenClaw container. No authentication required.
...
Transparent HTTP proxy to a running OpenClaw container. Forwards all HTTP methods to the container's internal address on port 18789. No authentication is required on the proxy itself — the OpenClaw instance's own token authentication handles access control.

<Note>WebSocket upgrades for the proxy are handled separately at the server level. The proxy rewrites the request path by stripping the `/api/openclaw/proxy/:agentId` prefix before forwarding.</Note>
