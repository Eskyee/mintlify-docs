...
The web-facing version endpoints (`GET /api/openclaw-version` and `GET /api/openclaw/version` on the web layer) normalize the `openclawVersion` value before returning it. If the backend reports the version as `"latest"` (which can happen when the `OPENCLAW_IMAGE` environment variable uses a floating tag), the endpoint replaces it with the current managed baseline version (currently `2026.4.11`).

This ensures the API always returns a concrete release number rather than an opaque tag, so you can reliably compare the running version against known releases.
...
<Note>This endpoint uses an 8-second request timeout when calling the backend. If the backend does not respond within that window, the fallback version is returned. The version normalization applies to both `GET /api/openclaw-version` and the web-layer `GET /api/openclaw/version` route — the backend-only `GET /api/openclaw/version` endpoint returns the raw derived version without normalization.</Note>
