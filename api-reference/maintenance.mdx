...
Pins the agent to the known-good OpenClaw image (`v2026.3.28`), reconfigures environment variables, and restarts the container with `doctor --fix`. Use this when an agent is broken after updating to an incompatible version.
...
{
  "success": true,
  "message": "Factory reset complete — pinned to ghcr.io/openclaw/openclaw:2026.3.28. Agent restarting with doctor --fix.",
...
<Note>Factory reset pins the agent to a known-stable release (`v2026.3.28`). After a factory reset, you can update to a newer version later through the normal update flow.</Note>
...
| Field | Type | Description |
|-------|------|-------------|
| `openclawVersion` | string | Current OpenClaw runtime version (currently `2026.3.28`) |
