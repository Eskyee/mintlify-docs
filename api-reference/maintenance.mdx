...
Requires session authentication and instance ownership. Returns runtime stats for a specific agent instance by probing the agent's gateway. Resource-level metrics (CPU, memory) are not yet exposed by the gateway and return placeholder values.
