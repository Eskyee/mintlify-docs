...
<Note>The orchestration API does not apply authentication middleware at the route level. All requests are subject to the general API rate limit of 120 requests per minute. Authentication may be enforced at the application mount level depending on your deployment configuration.</Note>
...
<Note>Individual tool call objects are not validated for required fields (`id`, `toolName`) at the API level. Malformed tool objects are passed to the executor and may produce runtime errors in the tool results.</Note>
