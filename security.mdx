...
Agentbot implements Google's [RISC (Risk Incident Sharing and Collaboration)](https://developers.google.com/identity/protocols/risc) protocol for enhanced OAuth security.
...
| Event | Action |
|-------|--------|
| `https://schemas.openid.net/secevent/risc/event-type/account-compromised` | Revoke all sessions for the affected user |
| `https://schemas.openid.net/secevent/risc/event-type/account-disabled` | Disable user account and revoke all sessions |
| `https://schemas.openid.net/secevent/risc/event-type/account-enabled` | Re-enable user account |
| `https://schemas.openid.net/secevent/risc/event-type/identifier-changed` | Revoke all sessions to force re-authentication |
| `https://schemas.openid.net/secevent/risc/event-type/sessions-revoked` | Revoke all active sessions |

<Note>The following RISC event types are defined in the specification but are not yet handled: `identifier-recycled`, `recovery-activated`, `recovery-information-changed`. These events are acknowledged with a `200` response but no action is taken.</Note>

### Implementation

Agentbot's RISC webhook endpoint:
...
The endpoint:
1. Decodes the SET (Security Event Token) JWT
2. Parses the security event type from the JWT payload
3. Looks up the affected user by email address
4. Takes appropriate action (session revocation, account status change, etc.)
...
1. **Immediate**: Revoke affected sessions and update account status if applicable
2. **Audit**: Log event for security review
...
2. Add the webhook URL: `https://agentbot.raveculture.xyz/api/auth/google/risc`
