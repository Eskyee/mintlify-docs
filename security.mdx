...
|--------|-------|---------|
| `Content-Security-Policy` | Restrictive policy with `default-src 'self'` and `script-src 'self'` (no `unsafe-eval`) | Limits sources for scripts, styles, images, and connections. Scripts are restricted to same-origin only — `unsafe-eval` is not permitted. |
