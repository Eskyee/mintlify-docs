...
The backend API enables CORS with configurable allowed origins. When the `ALLOWED_ORIGINS` environment variable is set (comma-separated list of domains), only those origins are permitted. When unset, all origins are allowed (`*`). Credentials are supported.
