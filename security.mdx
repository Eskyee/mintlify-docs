...
| Event | Action |
|-------|--------|
| `https://schemas.openid.net/secevent/risc/event-type/account-compromised` | Revoke all sessions, force re-authentication |
| `https://schemas.openid.net/secevent/risc/event-type/account-disabled` | Disable user access, notify via email |
| `https://schemas.openid.net/secevent/risc/event-type/account-enabled` | Re-enable user access |
| `https://schemas.openid.net/secevent/risc/event-type/identifier-changed` | Update email, invalidate old sessions |
| `https://schemas.openid.net/secevent/risc/event-type/identifier-recycled` | Treat as new account, clear old data |
| `https://schemas.openid.net/secevent/risc/event-type/recovery-activated` | Log recovery attempt, notify user |
| `https://schemas.openid.net/secevent/risc/event-type/recovery-information-changed` | Audit recovery method changes |
...
The endpoint:
1. Validates the JWT signature from Google
2. Parses the security event
3. Takes appropriate action (session revocation, user notification, etc.)
...
2. **Short-term**: Notify user via email
3. **Audit**: Log event for security review
4. **Recovery**: Guide user through re-authentication
