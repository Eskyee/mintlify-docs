...
```bash
curl https://x402-gateway-production.up.railway.app/health
```
...
curl -v https://x402-gateway-production.up.railway.app/api/sessions/inference
...
```bash
curl https://x402-gateway-production.up.railway.app/gateway/marketplace
```
...
```bash
curl https://x402-gateway-production.up.railway.app/gateway/marketplace/atlas
```
...
curl https://x402-gateway-production.up.railway.app/openapi.json
```

```bash
# Fetch the LLM-friendly text summary
curl https://x402-gateway-production.up.railway.app/llms.txt
```
...
```bash
curl https://x402-gateway-production.up.railway.app/gateway/fitness/atlas
```
...
```bash
curl https://x402-gateway-production.up.railway.app/gateway/pricing/atlas
```
...
```bash
curl -X POST https://x402-gateway-production.up.railway.app/gateway/colony/join \
...
```bash
curl -X POST https://x402-gateway-production.up.railway.app/gateway/marketplace/pay \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST https://x402-gateway-production.up.railway.app/gateway/settle/auto \
  -H "Content-Type: application/json" \
  -d '{
    "agentId": "atlas",
    "amount": 0.01,
...
Domain verified: `x402-gateway-production.up.railway.app`
...
```bash
curl https://x402-gateway-production.up.railway.app/gateway/guard
```
...
```bash
curl -X POST https://x402-gateway-production.up.railway.app/gateway/x402-node/settle \
  -H "Content-Type: application/json" \
  -d '{
...
The gateway URL defaults to `https://x402-gateway-production.up.railway.app` and can be configured via the `X402_GATEWAY_URL` environment variable.
