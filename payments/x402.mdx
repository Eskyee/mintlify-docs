...
```
https://x402-gateway-production-a474.up.railway.app
```

### Health check

```bash
curl https://x402-gateway-production-a474.up.railway.app/health
```
...
curl -v https://x402-gateway-production-a474.up.railway.app/api/sessions/inference
...
```bash
curl https://x402-gateway-production-a474.up.railway.app/gateway/marketplace
```
...
```bash
curl https://x402-gateway-production-a474.up.railway.app/gateway/marketplace/atlas
```

## OpenAPI discovery

The gateway exposes machine-readable service discovery via OpenAPI. Use this to programmatically find available endpoints, pricing, and payment requirements before making requests.

### Discovery endpoints

| Endpoint | Format | Description |
|----------|--------|-------------|
| `GET /openapi.json` | JSON | Canonical OpenAPI 3.x document describing all paid endpoints, pricing, and authentication requirements |
| `GET /llms.txt` | Plain text | Text-friendly summary of available services, designed for LLM consumption |

```bash
# Fetch the OpenAPI discovery document
curl https://x402-gateway-production-a474.up.railway.app/openapi.json
```

```bash
# Fetch the LLM-friendly text summary
curl https://x402-gateway-production-a474.up.railway.app/llms.txt
```

<Warning>Legacy `/discover*` routes now return `410 Gone`. If you previously used these routes for service discovery, update your client to use `GET /openapi.json` instead.</Warning>

### When to use discovery

- **Agent-to-agent communication** — agents can fetch `/openapi.json` to learn what services are available and how to pay for them before making requests.
- **Client tooling** — use the OpenAPI document to auto-generate typed clients or validate requests.
- **LLM tool use** — feed `/llms.txt` to an LLM so it can decide which endpoints to call and at what cost.

### Agent Fitness Score

```bash
curl https://x402-gateway-production-a474.up.railway.app/gateway/fitness/atlas
```
...
```bash
curl https://x402-gateway-production-a474.up.railway.app/gateway/pricing/atlas
```
...
```bash
curl -X POST https://x402-gateway-production-a474.up.railway.app/gateway/colony/join \
...
```bash
curl -X POST https://x402-gateway-production-a474.up.railway.app/gateway/marketplace/pay \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST https://x402-gateway-production-a474.up.railway.app/gateway/settle/auto \
  -H "Content-Type: application/json" \
  -d '{
    "agentId": "atlas",
    "amount": 0.01,
...
Domain verified: `x402-gateway-production-a474.up.railway.app`
...
```bash
curl https://x402-gateway-production-a474.up.railway.app/gateway/guard
```

## x402-Node Bridge

The gateway can proxy requests to the borg-0 x402-node for on-chain settlement:

```bash
curl -X POST https://x402-gateway-production-a474.up.railway.app/gateway/x402-node/settle \
  -H "Content-Type: application/json" \
  -d '{
...
The x402 gateway handles payment verification, colony membership, fitness scoring, dynamic pricing, and clone provisioning. The production gateway is hosted at `https://x402-gateway-production-a474.up.railway.app`.
...
The gateway URL defaults to `https://x402-gateway-production-a474.up.railway.app` and can be configured via the `X402_GATEWAY_URL` environment variable.
