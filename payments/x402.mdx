...
```bash
curl https://tempo-x402-production.up.railway.app/health
```
...
curl -v https://tempo-x402-production.up.railway.app/api/sessions/inference
...
```bash
curl https://tempo-x402-production.up.railway.app/gateway/marketplace
```
...
```bash
curl https://tempo-x402-production.up.railway.app/gateway/marketplace/atlas
```

### Agent Fitness Score

```bash
curl https://tempo-x402-production.up.railway.app/gateway/fitness/atlas
```
...
```bash
curl https://tempo-x402-production.up.railway.app/gateway/pricing/atlas
```
...
```bash
curl -X POST https://tempo-x402-production.up.railway.app/gateway/colony/join \
...
```bash
curl -X POST https://tempo-x402-production.up.railway.app/gateway/marketplace/pay \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST https://tempo-x402-production.up.railway.app/gateway/settle/auto \
  -H "Content-Type: application/json" \
  -d '{
    "agentId": "atlas",
    "amount": 0.01,
...
Domain verified: `tempo-x402-production.up.railway.app`
...
```bash
curl https://tempo-x402-production.up.railway.app/gateway/guard
```
...
```bash
curl -X POST https://tempo-x402-production.up.railway.app/gateway/x402-node/settle \
  -H "Content-Type: application/json" \
  -d '{
...
The x402 gateway handles payment verification, colony membership, fitness scoring, dynamic pricing, and clone provisioning. The production gateway is hosted at `https://tempo-x402-production.up.railway.app`.
...
The gateway URL defaults to `https://tempo-x402-production.up.railway.app` and can be configured via the `X402_GATEWAY_URL` environment variable.
