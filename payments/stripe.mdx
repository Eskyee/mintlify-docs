...
Redirect the user to the wallet top-up endpoint with an `amount` query parameter (in cents):
...
The endpoint returns a JSON response with the Stripe checkout URL and session ID. Redirect the user to the `url` to complete payment.

**Example**

```bash
# Redirect user to wallet top-up ($25)
window.location.href = '/api/wallet/top-up?amount=2500'
```
...
You can also fund your wallet by sending pathUSD directly to your wallet address on the Tempo network. Copy your wallet address from the wallet dashboard and transfer funds from any compatible wallet. No Stripe checkout is needed for direct transfers.
