...
<Warning>The legacy endpoint at `/api/stripe/webhook` is deprecated. It logs a deprecation warning and forwards requests to `/api/webhooks/stripe`. Update your Stripe dashboard webhook URL to the canonical endpoint. A `GET` request to the legacy endpoint returns `{ "status": "deprecated", "message": "Use /api/webhooks/stripe instead" }`.</Warning>
