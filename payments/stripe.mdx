...
<Note>The webhook signing secret is still verified on every request before idempotency is checked. Invalid signatures are rejected with `400` and never written to the processed-events store.</Note>
...
<Warning>Ensure your Stripe checkout session metadata includes a valid `userId` field. When `userId` is missing or does not match an existing user, the webhook can only update users that already exist in the database by email. If the email does not match any existing user, the subscription activation is skipped.</Warning>
