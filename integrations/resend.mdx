...
<Warning>This endpoint is intended to be called by Resend only. You must configure the `RESEND_WEBHOOK_SECRET` environment variable for signature verification on inbound emails. When the secret is not configured, inbound email processing returns `500`.</Warning>
