...
- `POST /api/deployments` — Deploy agent services on Render (auth required)
...
POST /api/provision (Render backend)
       ...
       ├── Create Render web service via Render API
       ├── Assign subdomain
       │
       ▼
...
2. **Backend:** Run `agentbot-backend/` on Render or any Express-compatible host (includes the inline scheduler — no separate worker needed)
...
| Render services | Isolated agent environments per user (provisioned via Render API) |
